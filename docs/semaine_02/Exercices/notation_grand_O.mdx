...
3. Écrivez un programme Python qui compare le **temps d’exécution** de deux fonctions que vous concevrez :

   * Une fonction `lente(n)` avec complexité `O(n^2)`
   * Une fonction `rapide(n)` avec complexité `O(n)`

4. Mesurez le **temps d’exécution** des deux fonctions pour des valeurs croissantes de `n` (ex. : 100, 200, 400, 800…)

5. Affichez un graphique comparatif avec `matplotlib`.
